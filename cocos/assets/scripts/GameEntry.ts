...
export class GameEntry extends Component {
  ...
  private renderInfo(): void {
    ...
    if (me) {
      const detail: number[] = me.captured ? [...me.captured] : [];
      this.drawCapturedPile(this.displayCaptured(me), detail);
    }
  ...
  private drawCapturedPile(stackCards: number[], detailCards: number[]): void {
    const cw = 54;
    ...
    const stackH =
      stackCards.length > 0
        ? cw * CARD_RATIO + (stackCards.length - 1) * step
        : cw * CARD_RATIO;
    const stackOffset = stackCards.length > 0 ? (stackCards.length - 1) * step : 0;
    const hitW = Math.max(stackW, 72) + 20;
    const hitH = stackH + 36;
    ...
    hit.setPosition(
      new Vec3(
        originX + stackW / 2,
        originY + stackOffset / 2 - (cw * CARD_RATIO) / 2 + 10,
        0
      )
    );
    hit.addComponent(UITransform).setContentSize(new Size(hitW, hitH));
    ...
    if (stackCards.length) {
      ...
      const empty = new Node("CapEmpty");
      empty.layer = Layers.Enum.UI_2D;
      this.infoNode.addChild(empty);
      empty.setPosition(new Vec3(originX + cw / 2, originY - (cw * CARD_RATIO) / 2, 0));
      empty.addComponent(UITransform).setContentSize(new Size(cw, cw * CARD_RATIO));
      const eg = empty.addComponent(Graphics);
      eg.strokeColor = new Color(201, 169, 97, 115);
      eg.lineWidth = 1.5;
      eg.roundRect(-cw / 2, -(cw * CARD_RATIO) / 2, cw, cw * CARD_RATIO, 6);
      eg.stroke();
    }
    const badge = new Node("CapBadge");
    badge.layer = Layers.Enum.UI_2D;
    this.infoNode.addChild(badge);
    badge.setPosition(
      new Vec3(originX + 32, originY + stackOffset + 18, 0)
    );
    badge.addComponent(UITransform).setContentSize(new Size(64, 22));
    const bg = badge.addComponent(Graphics);
    bg.fillColor = this.showCaptured
      ? new Color(184, 53, 43, 235)
      : new Color(8, 26, 20, 210);
    bg.roundRect(-32, -11, 64, 22, 11);
    bg.fill();
    bg.strokeColor = C.gold;
    bg.lineWidth = 1;
    bg.roundRect(-32, -11, 64, 22, 11);
    bg.stroke();
    addLabel(badge, `已吃 ${detailCards.length}`, 0, 0, 12, C.cream, true);

    if (!this.showCaptured) return;
    const mobile = sys.isMobile;
    const tw = mobile ? 72 : 88;
    const gap = 8;
    const innerPad = 14;
    const panelW = Math.min(DESIGN.width * 0.92, DESIGN.width - 24);
    const cols = Math.max(
      1,
      Math.min(
